...
const HomePage = () => {
  ...
  useEffect(() => {
    ...
    if (user.Location.Region === "Addis Ababa") {
      setSubCity(user.Location.SubCity || "");
      setCity("");
    } else {
...
console.log(error)
  ...
  📍 {user?.Location?.Region
    ? `${user.Location.Region} - ${user.Location.Region === "Addis Ababa"
        ? user.Location.SubCity || "Unknown SubCity"
    ...
    : "Addis Ababa"}
      ...
      {showLocationModal && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          ...
          >
            <h2 className="text-lg font-semibold text-slate-800 mb-1">Choose your location</h2>
            <p className="text-sm text-slate-500 mb-4">This helps us show nearby hospitals</p>
            ...
            {/* SUB-CITY FOR ADDIS ABABA */}
            {region === "Addis Ababa" && (
              <div className="mb-4">
                <label className="text-sm font-medium text-slate-600">Sub-City *</label>
                <select
                  value={subCity}
